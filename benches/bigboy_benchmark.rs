...
fn day_17_silver_benchmark(c: &mut Criterion) {
    let bigboy = test_utils::read_file_as_string(r"./test_data/day_17_p.txt").unwrap();

    c.bench_function(
        "day 17 calculate silver",
        |b| b.iter(|| day_17::part_1(&bigboy))
    );
}
fn day_17_gold_benchmark(c: &mut Criterion) {
    let bigboy = test_utils::read_file_as_string(r"./test_data/day_17_p.txt").unwrap();

    c.bench_function(
        "day 17 calculate gold",
        |b| b.iter(|| day_17::part_2(&bigboy))
    );
}

fn day_18_silver_benchmark(c: &mut Criterion) {
    let bigboy = test_utils::read_file_as_string(r"./test_data/day_18_p.txt").unwrap();

    c.bench_function(
        "day 18 calculate silver",
        |b| b.iter(|| day_18::part_1(&bigboy))
    );
}
fn day_18_gold_benchmark(c: &mut Criterion) {
    let bigboy = test_utils::read_file_as_string(r"./test_data/day_18_p.txt").unwrap();

    c.bench_function(
        "day 18 calculate gold",
        |b| b.iter(|| day_18::part_2(&bigboy))
    );
}
...
criterion_group!(day_17, day_17_silver_benchmark, day_17_gold_benchmark);
criterion_group!(day_18, day_18_silver_benchmark, day_18_gold_benchmark);
criterion_main!(day_18);